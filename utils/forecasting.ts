...
function projectRecurringToEOM(
  ...
  recurring.forEach(r => {
    ...
    const alreadyOccurred = matchingTransactions.length > 0
    
    // Find the last occurrence date if it already occurred
    let lastOccurrenceDate: Date | null = null
    if (alreadyOccurred && matchingTransactions.length > 0) {
      const dates = matchingTransactions.map(t => parseISO(t.date))
      lastOccurrenceDate = new Date(Math.max(...dates.map(d => d.getTime())))
    }
    ...
    // For weekly/biweekly, check if already occurred this month before projecting
    if (r.frequency === 'weekly') {
      if (alreadyOccurred && lastOccurrenceDate) {
        // Calculate days since last occurrence
        const daysSinceLastOccurrence = differenceInDays(today, lastOccurrenceDate)
        // Only project if at least 7 days have passed since last occurrence
        ...
        // Hasn't occurred this month, project based on remaining days
        const remainingWeeks = Math.floor(differenceInDays(monthEnd, today) / 7)
        projectedTotal += r.averageAmount * remainingWeeks
      }
    }
    ...
    if (r.frequency === 'biweekly') {
      if (alreadyOccurred && lastOccurrenceDate) {
        // Calculate days since last occurrence
        const daysSinceLastOccurrence = differenceInDays(today, lastOccurrenceDate)
        // Only project if at least 14 days have passed since last occurrence
